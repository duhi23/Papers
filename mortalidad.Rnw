...
\emph{\scriptsize *Escuela Politécnica Nacional, Departamento de Matemática, Quito, Ecuador}\\
{\scriptsize \url{e-mail: diego.huaraca@epn.edu.ec}}\\
...
El número de habitantes en Ecuador se ha ido incrementando paulatinamente al punto de triplicarse en los últimos $40$ años. Además, se ha observado que la esperanza de vida se ha incrementado, tal como se muestra en las siguientes pirámides poblacionales:
\begin{figure}[H]
\centering
...
Para el estudio de el fenómeno de desaceleración, se emplea la fuerza de mortalidad de la función de supervivencia $S(x)$ definida como:
\begin{equation}
\mu(x)=\frac{d}{dx}lnS(x)=\frac{f_x(x)}{S_x(x)}
\end{equation}
Esta función representa la tasa instantánea de mortalidad para un individuo que sobrevive a la edad $x$. Debido que las tablas de vida empleadas para el estudio no contaban con dicho valor de esta función, se estimó los valores aproximados utilizando la siguiente expresión:
\begin{equation}{
\label{ecu:001}
\mu(x)=-ln(1-q_x)}
\end{equation}
...
\section{Metodología}
Para determinar la edad en la que se presenta el fenómeno de desaceleración de la mortalidad se utilizó: la tasa de envejecimiento de la tabla de vida, y además, la primera y segunda derivada de la función fuerza de mortalidad.
...
Evidenciamos que de acuerdo a los criterios de información de Akaike y Bayesiano el modelo logístico resulta ser el más adecuado tanto para hombres como para mujeres. Por tal razón, calculamos las funciones de velocidad y aceleración de la mortalidad para obtener las edades en las cuales inicia el fenómeno de la desaceleración.

\begin{figure}[H]
\centering
	\includegraphics[width=0.85\linewidth]{vya_logistico_hombres.pdf}
	\caption{Velocidad $\&$ aceleración - Hombres.}
\end{figure}

\begin{figure}[H]
\centering
	\includegraphics[width=0.85\linewidth]{tvya_logistico_mujeres_1991_trio.pdf}
	\caption{Velocidad $\&$ aceleración - Mujeres.}
\end{figure}
...
\centering
	...
	\caption{Tasa de envejecimiento - Hombres $\&$ Mujeres} 
\end{figure}
...
\begin{enumerate}
      ...
      \item La diferencia de $3$ años en la edad de inicio de desaceleración de la mortalidad entre hombres y mujeres, concuerda con otros estudios realizados, pues generalmente la esperanza de vida de las mujeres es mayor que de los hombres, de donde a su vez la edad de inicio de la desaceleración de la mortalidad será mayor para las mujeres que para los hombres.
\end{enumerate}
